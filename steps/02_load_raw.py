...
from utils.constants import POS_SCHEMA, CUSTOMER_SCHEMA
from utils import snowpark_utils

POS_TABLES = ['country', 'franchise', 'location', 'menu', 'truck', 'order_header', 'order_detail']
CUSTOMER_TABLES = ['customer_loyalty']
TABLE_DICT = {
    "pos": {"schema": POS_SCHEMA, "tables": POS_TABLES},
    "customer": {"schema": CUSTOMER_SCHEMA, "tables": CUSTOMER_TABLES}
}
...
def validate_raw_tables(session):
    ...
    for tname in POS_TABLES:
        print('{}: \n\t{}\n'.format(tname, session.table(f'{POS_SCHEMA}.{tname}').columns))

    for tname in CUSTOMER_TABLES:
        print('{}: \n\t{}\n'.format(tname, session.table(f'{CUSTOMER_SCHEMA}.{tname}').columns))
