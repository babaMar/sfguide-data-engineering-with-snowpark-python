...
from utils.constants import POS_SCHEMA, HARMONIZED_SCHEMA
from utils import snowpark_utils


def create_pos_view(session):
    session.use_schema(f'{HARMONIZED_SCHEMA}')
    order_detail = session.table(f"{POS_SCHEMA}.ORDER_DETAIL").select(
    ...
    order_header = session.table(f"{POS_SCHEMA}.ORDER_HEADER").select(
        F.col("ORDER_ID"),
        F.col("TRUCK_ID"),
        F.col("ORDER_TS"),
    ...
    truck = session.table(f"{POS_SCHEMA}.TRUCK").select(
        F.col("TRUCK_ID"),
        F.col("PRIMARY_CITY"),
        F.col("REGION"),
        F.col("COUNTRY"),
    ...
    menu = session.table(f"{POS_SCHEMA}.MENU").select(
        F.col("MENU_ITEM_ID"),
        F.col("TRUCK_BRAND_NAME"),
        F.col("MENU_TYPE"),
        F.col("MENU_ITEM_NAME")
    )
    franchise = session.table(f"{POS_SCHEMA}.FRANCHISE").select(
        F.col("FRANCHISE_ID"),
        F.col("FIRST_NAME").alias("FRANCHISEE_FIRST_NAME"),
        F.col("LAST_NAME").alias("FRANCHISEE_LAST_NAME")
    )
    location = session.table(f"{POS_SCHEMA}.LOCATION").select(F.col("LOCATION_ID"))
    ...
    t_with_f = truck.join(
        franchise,
        truck['FRANCHISE_ID'] == franchise['FRANCHISE_ID'],
        rsuffix='_f'
    )
    oh_w_t_and_l = order_header.join(
        t_with_f,
        order_header['TRUCK_ID'] == t_with_f['TRUCK_ID'],
        rsuffix='_t'
    ).join(
        location,
        order_header['LOCATION_ID'] == location['LOCATION_ID'],
        rsuffix='_l'
    )
    final_df = order_detail.join(
        oh_w_t_and_l, order_detail['ORDER_ID'] == oh_w_t_and_l['ORDER_ID'],
        rsuffix='_oh'
    ).join(
        menu,
        order_detail['MENU_ITEM_ID'] == menu['MENU_ITEM_ID'],
        rsuffix='_m'
    )
